...
import type { DiagnosticOptions, ToolDefinition } from '@reaatech/mcp-server-doctor-core';
    ...
    switch (this.options.auth) {
      ...
      case 'oauth':
        if (this.options.oauthClientId && this.options.oauthClientSecret) {
          ...
          headers.Authorization = `Basic ${Buffer.from(`${this.options.oauthClientId}:${this.options.oauthClientSecret}`).toString('base64')}`;
