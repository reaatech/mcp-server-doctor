...
describe('Transport Implementations', () => {
  ...
  describe('StdioTransport', () => {
    ...
    it('logs stderr data', async () => {
      ...
      // Just verify it doesn't throw; logger is tested via side effects in observability tests
      expect(transport).toBeDefined();
    });
    ...
    it('rejects pending requests when process exits after connect', async () => {
      ...
      // Fire all spawn handlers to simulate process spawn
      handlers.get('spawn')?.forEach((cb) => cb());
      await connectPromise;
      const pendingPromise = transport.sendRequest('slow', {});
      // Fire the last-registered exit handler (the one that rejects pending requests)
